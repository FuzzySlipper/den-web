...
describe('normalizeStoredPiCrewAdminBaseUrl', () => {
  it('migrates legacy browser-local Pi Crew defaults to the same-origin proxy', () => {
    ...
    expect(normalizeStoredPiCrewAdminBaseUrl('http://192.168.1.22:9237/', '/pi-crew-admin-api')).toBe('/pi-crew-admin-api');
  });

  it('preserves explicit non-legacy admin endpoints', () => {
    expect(normalizeStoredPiCrewAdminBaseUrl('http://192.168.1.99:9237/', '/pi-crew-admin-api')).toBe('http://192.168.1.99:9237');
  });
