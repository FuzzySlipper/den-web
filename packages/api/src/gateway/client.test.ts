...
describe('getWorkerPoolLobbyPresence', () => {
  ...
  it('returns an empty successor-safe projection without polling the retired Channels lobby', async () => {
    const fetchMock = vi.fn();
    vi.stubGlobal('fetch', fetchMock);
  ...
  it('keeps the degraded projection stable across calls', async () => {
    const presence = await getWorkerPoolLobbyPresence();

    expect(presence.availableCount).toBe(0);
    expect(presence.totalCandidateCount).toBe(0);
    expect(presence.members).toEqual([]);
  });
});
...
describe('Gateway polling client', () => {
  ...
  it('does not poll the retired Channels worker-pool lobby route', async () => {
    const fetchMock = vi.fn();
    vi.stubGlobal('fetch', fetchMock);
